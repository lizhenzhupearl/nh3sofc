# Karpathy's CLAUDE.md — 10 Rules for Reliable LLM Coding

> Source: Andrej Karpathy, June 2026
> Reference copy for NH3SOFC project. See CLAUDE.md for the condensed version integrated into our harness.
...
**Dead abstraction.** Interfaces with one implementation. Abstract base classes with one child. Generic type parameters that are only ever instantiated with one type. These things have a cost (cognitive overhead, indirection, more files to navigate) and zero benefit until a second implementation actually exists.
...
**Match the existing style.** If the file uses single quotes, use single quotes. If the file uses `snake_case`, use `snake_case`. If the file has no semicolons, don't add semicolons. If the file uses `var` (yes, even in 2025), use `var` in your addition unless the user asked you to modernize. Consistency within a file beats your personal preference.
...
**Don't reformat.** Don't run prettier on a file that wasn't formatted with prettier. Don't change indentation from 4 spaces to 2. Don't reorder imports alphabetically if they weren't alphabetically before. Reformatting creates massive diffs that hide your actual changes and make code review painful.
...
**If you can't write a test, say why.** Sometimes the architecture makes testing hard. That's useful information. "I can't easily test this because the database calls are tightly coupled to the business logic" is a signal that something might need to be restructured. Don't just stop testing and hope.
...
**Reproduce first.** Before you change anything, make sure you can reproduce the problem. If you can't reproduce it, you can't verify your fix. "I think this should fix it" is not debugging; it's guessing.
...
When you do add a dependency, say why. "I'm adding zod because this project needs runtime schema validation and there's nothing in the existing dependencies that does this" is fine. Silently adding package to package.json is not.
...
**The Knowledge Hallucination.** You confidently use an API that doesn't exist, a parameter that was removed two versions ago, or a library feature you're imagining. If you're not 100% sure a method exists with that exact signature, say so. Check the docs. Look at the actual source code in the project.
...
These guidelines work when they produce fewer unnecessary changes in diffs, fewer rewrites due to overcomplification, and clarifying questions come before implementation rather than after mistakes.
